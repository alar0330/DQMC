...
MultReturn processUserPrompt(const int);
double runSim(int, int, double, double, unsigned, ptrV, double*, unsigned);

double potentialSHO1D(const Walker& w);
double potentialSHO3D(const Walker& w);
double potentialMorse(const Walker& w);
double potentialH(const Walker& w);
double potentialH2p(const Walker& w);
...
int main() {
  ...
  // -- user prompt start --
  cout << "/// This is DQMC -- v0.7 alpha (14.08.2017). \n\n";
...
// -(updated v0.7a)-
MultReturn processUserPrompt(const int pot) {
